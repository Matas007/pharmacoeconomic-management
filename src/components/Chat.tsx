...
export default function Chat() {
  ...
  const handleSetupPin = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setConfirmPin('')
        setPinError('')
        // Po setup, automatiškai bandyti prisijungti
        setTimeout(() => fetchMessages(), 500)
      } else {
        setPinError(data.error || 'Klaida kuriant PIN')
      }
    } catch (error) {
  ...
  const handleRoomSelect = async (room: ChatRoom) => {
    ...
    // Jei USER - tikrinti ar reikia setup PIN
                  ...
                  <div className="w-full max-w-sm">
                    <div className="text-center mb-4 sm:mb-6">
                      ...
                      <h3 className="text-lg sm:text-xl font-semibold text-gray-900 mb-2">
                        {needsSetup ? 'Sukurkite savo PIN kodą' : 'Įveskite PIN kodą'}
                      </h3>
                      <p className="text-sm sm:text-base text-gray-600">
                        {needsSetup ? 'Pirma karta - sukurkite 4 skaitmenų PIN' : '4 skaitmenų PIN kodas'}
                      </p>
                    </div>
                      ...
                      <>
                        ...
                        <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                          <p className="text-xs text-gray-600">
                            <strong>ℹ️ Svarbu:</strong> Admin naudoja PIN 5678 visiems USER chatams. Jūsų PIN tik jums!
                          </p>
                        </div>
                      </>
                    ) : (
